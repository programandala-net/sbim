...
" This Vim command program converts
" S*BASIC source code written in the SBim format
" to an ordinary S*BASIC file ready to be loaded by a
" S*BASIC interpreter.
...
" 2011-08-13 First version, named sb2sbasic.vim, based on vim2mb.vim ("Vim to MasterBASIC"), by the same author. [vim2mb was the origin of MBim].
...
" 2011-09-01 Renamed to sbim2bas.vim; own renum function instead of using that of line_numbers.vim; all functions renamed with the "SBim" prefix.
" 2011-09-26 Renamed to sbim2sb.vim; some functions renamed; some comments improved.
" 2011-10-04 Block comments fixed with '\{-}'; 'silent' added to many commands; 'e' flag added to substitutions; custom messages added.
" 2012-01-25 New function SBimLabels(), based on BBimLabels() from BBim. Fixed: lonely labels had to be joined with the next line; optional final ":" removed with labels.
...
" 2012-01-28 New feature: #firstline command to define the first line that will be used to renumber the final program in S*BASIC.
" 2012-01-28 Improvement: The S*BASIC buffer is actually closed with "bw" ("wq" kept it on the list).
" 2012-01-29 Improvement: Only one regex, simpler and fixed, to remove empty lines (the old three regex left the first line of the file blank).
" 2012-01-29 Fixed: Spaces or tabs at the end of line, after the split bar, are ignored.
" 2012-02-01 Bug found. The label translation loop doesn't end in some cases.
" 2012-02-15 Fixed? The 'c' parameter was removed from search() in the label translation loop.
...
function! SBimClean()
  ...
  echo 'Indentation and blanks removed.'
	...
	" silent %s,|\n,,e " Join the splitted lines (the vertical bar is used, because the backslash is a printing separator is S*BASIC)
...
function! SBimLabels()
  ...
  " Translate the line labels.
	...
	" Create an empty dictionary to store the line numbers of the labels; the labels will be used as keys:
	let l:lineNumber={}
	" Go to the top of the file:
	normal gg
